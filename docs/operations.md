...
| `query` 가 승인 0 — `판정 집계: 승인 0 · 보류 N · 기각 N` | 질문의 말이 Card 키워드·별칭과 겹치지 않음. Card 의미 생성기를 설정하지 않았다면 그 어휘가 원문에서 잘라낸 조각뿐이라 더 자주 어긋납니다 | 질문을 문서의 말로 바꿔 보고, 그래도 안 잡히면 `CONTEXTCTL_CARD_MEANING_BASE_URL` / `_MODEL` / `_API_KEY` 를 설정하고 **다시 `ingest`** (의미는 수집 시점에 굳습니다) → [설정](configuration.md#card-의미-생성기-선택) |
