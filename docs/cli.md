...
기본 로컬 임베딩 자산을 내려받아 설치합니다. 문서 검색과 Card 선택은 논리적으로 독립된
계층이지만, 둘 다 기본 로컬 프로필이면 같은 검증 자산과 읽기 전용 추론 세션을 사용할 수 있습니다.
내려받기 전에 저장소·리비전·라이선스·총 용량을 보여주고 동의를 묻습니다. `--yes`는 그 질문에
미리 동의했다는 뜻입니다. 터미널이 아닌 환경(CI·파이프)에서는 동의를 추정하지 않으므로 새로
설치하려면 반드시 `--yes`를 써야 합니다. 이미 정확한 revision이 설치돼 있으면 묻지 않고 성공합니다.

새 상태이거나 이관이 끝나 살아 있는 로컬 프로필 참조가 없는 상태에서 두 계층을 모두 원격으로
설정했다면 이 자산은 필요하지 않습니다. 승인 Card가 이전 로컬 문서 Scope를 계속 가리키면
설정만 원격으로 바꿔도 그 Scope를 검색할 자산은 계속 필요합니다.

`--source-directory` 는 미리 받아둔 디렉터리에서 설치합니다(네트워크 없음). 파일을 복사해
상태를 바꾸므로 비대화형 실행에는 이 경우도 `--yes`가 필요합니다. **같은 SHA-256 검증을
통과해야 하므로** 다른 모델을 넣는 우회로는 아닙니다.
...
`doctor`는 활성 두 임베딩 계층뿐 아니라 승인 Scope가 아직 참조하는 이전 문서 프로필의 바인딩도
조립하고, 필요한 로컬 자산의 이름·크기·프로필 일치를 확인합니다. `doctor --deep`은 파일 내용
다이제스트까지 검사합니다. `query`와 `serve`도 시작할 때 같은 전체 내용 검증을 통과해야 합니다.
...
다시 하지 않습니다. 같은 수집 정책·임베딩 프로필에서 기존 의미 단위의 본문만 바뀌면 해당
청크만 다시 임베딩하고 같은 Card에 새 버전이 쌓입니다. 새 의미 단위는 새 Card를 만들고, 사라진
단위의 Card는 회수됩니다. 프로필이나 수집 정책의 호환 경계가 바뀌면 전체 색인을 재구축합니다.
색인이 사라진 경우의 조치는
...
응답에는 **선택 판정 집계**(승인·보류·기각)가 함께 나옵니다. 선택된 Card는 식별할 수 있지만,
기각된 Card의 식별자와 개별 판정 사유는 기본 응답에 싣지 않습니다.
...
`backup create`는 두 SQLite 저장소, Source 등록 파일(`sources.json`)과 Ingestion 상태에 기록된
모든 불변 게시 Index 버전의 Qdrant 컬렉션을 **하나의 묶음**으로 저장합니다. 현재 포인터가 아닌
옛 버전도 승인·되돌리기 가능한 Card가 참조할 수 있어 포함합니다. 원본 Markdown 파일은 포함하지
