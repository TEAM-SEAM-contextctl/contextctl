...
> **각주 (SEAM-79).** 이 결정의 전제 — 물리 좌표가 승인 Card read model에 존재하며 가이드가
> 그것을 제외한다 — 는 SEAM-79에서 사라졌다. `ApprovedDocumentIndexRef`가
> `connectorId`·`accessHandle`을 더 이상 담지 않으므로 제외는 규율이 아니라 타입의 성질이
> 됐다. 결정은 뒤집히지 않고 더 강하게 보장된다.
