...
`contextctl doctor`는 이 값을 사용해 기존 상태의 내용을 바꾸지 않고 검사하며, 새 홈이나 DB를
만들고 기본값을 기록하지 않습니다. 단, 디렉터리 권한 확인에는 즉시 제거하는 임시 경로를 씁니다.
새 설치의 DB 없음 경고는 정상이며, 첫 상태 변경 명령을 실행하기 전에 운영에 사용할 두 값을
확정하십시오.
