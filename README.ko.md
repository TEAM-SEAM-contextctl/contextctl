...
공개 데모 문서 5개와 봉인 홀드아웃 25문항으로, 같은 실행이 Qdrant에 게시한 동일한 Chunk와
벡터를 읽는 전역 BM25 + Dense + RRF 기준선과 비교했습니다.
...
생성 API를 연결하지 않았으므로 토큰이 아니라 원문 문자 수 기준이며, 일반 RAG보다 항상 낫다는 근거가 아닙니다.

[평가 방법, 전체 지표와 재현 절차](docs/benchmark.md)
