...
일반적인 문서 RAG는 전체 Chunk에서 질의와 가까운 것을 찾습니다. contextctl은 먼저 사람이 승인한 Card로 검색 범위를 정하고 그 안에서만 찾습니다. 그래서 전달되는 문맥이 줄어듭니다.

공개 데모 문서 5개와 봉인 홀드아웃 25문항으로, 같은 실행이 Qdrant에 게시한 동일한 Chunk와 벡터를 읽는 전역 BM25 + Dense + RRF 기준선과 비교했습니다.

| 지표 | 기준선 | contextctl |
| --- | ---: | ---: |
| 평균 전달 원문 문자 | 1,231 | 218 |
| 무관 Chunk 비율 | 84% | 4% |
| 답할 수 없는 질의 거절 | 0% | 100% |
| 필수 사실 포함률 | 100% | 90% |
| 검색 p95 | 63.34ms | 128.71ms |

전달 문맥은 82.26% 줄고 답할 수 없는 질의 5건은 모두 닫혔습니다. 그 대가로 답변 가능한 질의 20건 중 2건의 근거를 놓쳤고 검색 지연이 늘었습니다.
검색 산출물 품질은 필수 사실 포함률·관련 Chunk 회수율·순위·무관 문맥 비율로 검증했습니다. contextctl은 최종 답변을 만들지 않으므로 비교 단위는 호출자 LLM의 토큰이 아니라 제품이 전달한 원문 문자 수입니다.
이 결과는 공개 데모 범위의 측정이며 일반 RAG보다 항상 낫다는 주장은 아닙니다. [평가 방법과 전체 지표](docs/benchmark.md)
...
| [효용성 벤치마크](docs/benchmark.md) | Hybrid RAG 비교 조건, 결과, 한계와 재현 절차 |
