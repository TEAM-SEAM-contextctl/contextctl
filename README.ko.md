...
| **디스크** | 임베딩 모델 **396.1 MiB(약 415 MB)** — 기본 로컬 실행 기준. 새 상태이거나 이관을 끝낸 상태에서 두 계층을 모두 원격으로 쓰면 필요 없습니다. 이전 로컬 프로필로 게시된 Scope를 승인 Card가 계속 참조하면 그 참조를 끝낼 때까지 자산이 필요합니다 |
...
GitHub 설치기는 영어와 한국어를 지원합니다. `CONTEXTCTL_LOCALE=en|ko`로 명시할 수 있고,
없으면 `LC_ALL`, `LC_MESSAGES`, `LANG` 순서로 판단합니다. 알 수 없는 locale의 기본값은
영어입니다.
...
위 ID 는 읽기 쉽도록 중간을 `…`으로 줄였습니다. 새 상태나 파괴적 재구축에서는 다른 ID가
발급될 수 있습니다. 같은 영속 상태 안에서는 재시도·재시작·일반 재수집이 동일한 논리 대상의
ID를 보존합니다.
