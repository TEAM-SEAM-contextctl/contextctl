...
export class MarkdownFileSourceAdapter implements SourceAdapter {
  ...
  async detectChange(
    ...
    return snapshot.contentDigest === previousToken
      ? { status: "unchanged", token: snapshot.contentDigest }
      : { status: "changed", token: snapshot.contentDigest };
  }

  async observe(context: SourceAdapterContext): Promise<SourceObservationAttempt> {
    const snapshot = await this.#readSnapshot(context);
    return {
      status: "changed",
      payload: snapshot,
  ...
  async #readSnapshot(
    ...
    await inspectReadableFile(configuration, context.signal);
    try {
      const bytes = await readFile(configuration.path, {
        signal: context.signal,
      });
      if (bytes.byteLength > configuration.maxBytes) {
        throw new SourceAdapterFault("invalid_format");
      }
      let decoded: string;
      try {
